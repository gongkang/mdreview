...
- CLI 能操作的主要行为，在 App 菜单或设置页中也能找到对应入口。
- 界面保持简洁清爽，接近 GitBook 的文档阅读体验，不使用复杂底色、装饰性渐变或花哨视觉元素。
- 默认用户界面使用中文，尤其是菜单、设置项、错误提示和空状态文案。
...
使用 Swift + AppKit 为主，必要时用 SwiftUI 承载设置页面。
...
- CLI 收到 ack 后退出。
- Socket 路径固定为 `$TMPDIR/mdreview-$UID.sock`，并只允许当前用户访问。
- CLI 启动 App 后最多等待 5 秒连接 socket；连接超时则报错。
- 如果连接失败且 socket 文件存在，CLI 先尝试清理 stale socket，再启动 App 并重试一次。
- Ack 超时为 2 秒；超时后 CLI 输出中文错误并退出非零。
...
- `文件 > 打开文件...`：打开 Markdown 文件为 tab。
- `文件 > 打开文件夹...`：打开目录 workspace，默认替换当前窗口。
- `文件 > 在新窗口中打开文件夹...`：对应 `mdreview docs --new-window`。
- `文件 > 关闭标签页`
- `文件 > 关闭窗口`
- `视图 > 显示/隐藏文件`
- `视图 > 显示/隐藏大纲`
- `视图 > 重新载入当前文档`
- `窗口`：使用系统标准窗口行为，用户可见标题使用中文。
- `mdreview > 设置...`
...
窗口采用 Typora / Sublime 之间的折中模型：原生窗口 + 应用内文档标签页 + 左侧双层导航 + WKWebView 正文阅读区。

视觉原则：

- 整体参考 GitBook：清晰、克制、以阅读为中心。
- 主阅读区使用干净的白色或系统默认背景，避免大面积彩色底、渐变、纹理和装饰图形。
- Files 和 Outline 使用轻量分隔线、系统灰阶和少量 accent selection，不做卡片化或 dashboard 化。
- 正文区域保持舒适行宽，默认内容宽度约 760-860px，留出稳定边距。
- 顶部文档标签页和菜单优先使用 macOS 原生视觉，不自定义成重色块工具栏。
- 代码块、提示块、错误块可以有轻微底色，但只用于语义区分，不能主导页面色彩。
- 深色模式跟随系统，但仍保持低对比装饰和文档优先。

布局：

- 顶部：应用内文档标签页。MVP 不使用 macOS 系统窗口 tab 作为 Markdown 文件 tab，以免窗口 tab 行为和文档 tab 行为混淆。
- 左侧第一层：Files。
- 左侧第二层：Outline。
- 右侧：正文阅读区。
...
- 替换 workspace 时，关闭当前窗口内所有旧文档标签页，并打开新 workspace 的默认文档。MVP 不保留旧 workspace 的 tabs，也不混合保留外部单文件 tabs。

单文件模式：

- 没有 workspace 时完全隐藏 Files 列，窗口变为两列布局：左侧只有 Outline，右侧为正文阅读区。
- Outline 仍显示当前 active tab 的大纲。
- 如果窗口已有 workspace，单文件打开为该窗口的 tab，不替换 workspace。
- 单文件两列布局中，Outline 和正文之间仍支持拖拽调整宽度。
...
4. App 关闭当前窗口原有 tabs，清空旧 workspace 状态。
5. App 扫描 Markdown 文件树。
6. App 选择默认文档：根目录 README 优先，否则按相对路径排序的第一个 Markdown 文件。
7. 默认文档打开为 tab。
8. Files 显示目录树，Outline 显示 active tab 大纲。

### 新窗口打开目录

1. 用户运行 `mdreview docs --new-window` 或菜单 `文件 > 在新窗口中打开文件夹...`。
2. App 创建新窗口。
3. 新窗口绑定该 workspace。
4. 默认文档打开为 tab。
...
- `scrollToHeading`: `{ id }`
- `setResourceBase`: `{ rootPath, allowedPaths }`
...
- 本地图片或资源路径必须经原生层解析和授权。MVP 使用自定义 `mdreview-resource://` scheme 加 `WKURLSchemeHandler` 加载相对资源；renderer 将允许的相对资源 URL 重写到该 scheme，handler 基于当前文档路径和 workspace root 做 realpath containment，禁止访问授权范围外文件。
...
- 路径不存在：输出 `路径不存在：<path>`，退出非零。
- 路径不是文件或目录：输出明确错误，退出非零。
- App 找不到：提示安装 App 或运行开发安装命令。
- IPC 失败：启动 App 后重试一次；仍失败则用中文输出失败原因。
...
- 重复文件聚焦已有 tab。
- workspace 替换。
- workspace 替换时关闭旧 tabs。
- 单文件无 workspace 时隐藏 Files，只保留 Outline 和正文两列。
- Files / Outline 宽度持久化。
- 设置页默认值和更新。
...
- 同一文件重复打开聚焦已有 tab。
- 单文件无 workspace 时不显示 Files 列。
- 左侧 Files / Outline 可拖拽调整宽度。
- 默认界面观感接近 GitBook 式清爽文档工具，没有花哨底色、渐变背景或无关装饰。
